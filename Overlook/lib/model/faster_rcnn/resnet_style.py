...
class resnet(_fasterRCNN):
    def __init__(self, classes, num_layers=101, args=None,pretrained=False, class_agnostic=False,imdb=None):
    ...
    def _init_modules(self):
        ...
        self.RCNN_base2 = nn.Sequential(resnet.maxpool, resnet.layer1)
        ...
        if cfg.RESNET.FIXED_BLOCKS >= 3:
            for p in self.RCNN_base4.parameters():
                p.requires_grad = False
        if cfg.RESNET.FIXED_BLOCKS >= 2:
            for p in self.RCNN_base3.parameters():
                p.requires_grad = False
        if cfg.RESNET.FIXED_BLOCKS >= 1:
            for p in self.RCNN_base2[0].parameters():
                p.requires_grad = False
